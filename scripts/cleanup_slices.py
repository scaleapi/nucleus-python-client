...
def cleanup_slices(api_key, dataset_id):
    ...
    for slc_id in dataset.slices:
        ...
        if any(
            kw in slc_name.lower()
            for kw in ["test", "night", "police", "truck"]
        ):
            log.info(
                "Deleting Slice '%s' because it contains night/police/truck keyword. Slice id: %s",
                slc_name,
                slc_id,
            )
            client.delete_slice(slc_id)

        elif len(slc_info["dataset_items"]) < 1000:
            log.info(
                "Deleting Slice '%s' because it contains fewer than 1000 items. Slice id: %s",
                slc_name,
                slc_id,
            )
            client.delete_slice(slc_id)
