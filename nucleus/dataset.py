...
class Dataset:
    ...
    def deduplicate(
        ...
        asynchronous: bool = False,
    ) -> Union[DeduplicationResult, DeduplicationJob]:
        ...
        .. note::
            ...
            - For very large datasets, this operation may take significant time.
              Pass ``asynchronous=True`` to avoid HTTP timeouts.
        ...
        Parameters:
            ...
            asynchronous: When False (default), runs dedup synchronously and
                returns a :class:`DeduplicationResult` directly. When True,
                kicks off a background job and returns a
                :class:`DeduplicationJob` you can poll/await — recommended for
                large datasets where the sync HTTP request would otherwise
                time out.

        Returns:
            :class:`DeduplicationResult` when ``asynchronous=False``, or a
            :class:`DeduplicationJob` when ``asynchronous=True``. Call
            ``job.result()`` on the latter to block and unpack the result.
    ...
    def deduplicate_by_ids(
        ...
        Parameters:
            ...
            asynchronous: When False (default), runs dedup synchronously and
                ...
                kicks off a background job and returns a
                :class:`DeduplicationJob` you can poll/await — recommended for
                large id lists where the sync HTTP request would otherwise
                time out.
