...
def construct_append_payload(dataset_items: List[DatasetItem], force: bool = False) -> dict:
    items = []
    for item in dataset_items:
        items.append(item.to_payload())

    return {ITEMS_KEY: items} if not force else {ITEMS_KEY: items, FORCE_KEY: True}
